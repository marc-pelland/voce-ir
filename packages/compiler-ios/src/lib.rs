    ...
    match type_name {
        ...
        "TextNode" => {
            ...
            swift.push_str(&format!(
                "{pad}    .font(.system(size: {size:.0}, weight: .{wt}))\n",
        ...
        "MediaNode" => {
            ...
            swift.push_str(&format!(
                "{pad}AsyncImage(url: URL(string: \"{src}\")) {{ image in\n"
            ));
            swift.push_str(&format!(
                "{pad}    image.resizable().aspectRatio(contentMode: .fill)\n"
            ));
