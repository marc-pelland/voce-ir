...
| S79 (agent capability surface) | In progress — A1 (`voce skills`) + A2 (`voce doctor` Slice 1) + A3 (`voce graph`) + A4 Slice 1 (JSON Schemas for all 3 envelopes, schemars-derived, drift-gated + live-conformance tested) shipped. `docs/schema/contract/v1/` with policy README. Remaining: A1 Slice 2 (MCP parity for the new surfaces — wires into shipped `@voce-ir/cli-chat`/`@voce-ir/mcp-server` per S65/S66), A2 Slice 2 (IR-set walk), A4 Slice 2 (validator + perf-report schemas), A5 conformance runner, Part B |
