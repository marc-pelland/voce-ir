...
| S79 (agent capability surface) | In progress — A1 (`voce skills`) + A1 Slice 2 (MCP parity: 3 new tools, 19→22) + A2 Slice 1+2 (`voce doctor`, incl. opt-in `--ir-set` walk) + A3 (`voce graph`) + A4 Slice 1+2 (JSON Schemas for skills/graph/doctor/perf-report) + B2 (`voce fix --until-clean --plan`) + **B4 contract-as-only-interface guarantee (7 agent-task scenarios + cross-envelope drift gate)** shipped. 5 contract envelopes, all drift-gated + live-conformance + completeness-tested. Remaining: A4 Slice 3 (validator-output typed-envelope refactor), A5 conformance runner (overlaps S91), B3 agent-authorability lint |
