...
| S79 (agent capability surface) | In progress — A1 (`voce skills`) + A3 (`voce graph`) + A4 Slice 1 (JSON Schemas for both envelopes, schemars-derived, drift-gated + live-conformance tested) shipped. `docs/schema/contract/v1/` with policy README. Remaining: A1 Slice 2 (MCP parity), A2 doctor, A4 Slice 2 (validator + perf-report schemas), A5 conformance runner, Part B |
