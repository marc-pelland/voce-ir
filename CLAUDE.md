...
| S79 (agent capability surface) | **CLOSED** — A1+A1S2 (skills + MCP parity, 19→22 tools) + A2 (doctor +`--ir-set`) + A3 (graph) + A4 Slice 1+2 (5 schema-locked envelopes: skills/graph/doctor/perf-report/fix-plan) + B2 (fix --until-clean --plan) + B4 (contract-completeness guarantee + cross-envelope drift gate). Deferred: A5 folded into S91; A4 Slice 3 + B3 → S79b |
| S91 (conformance spec + certification) | In progress — Slice 1 shipped: `voce conformance run --target <id>` runner + `Level`/`Profile` types + 6th contract envelope (`conformance.schema.json`, drift-gated + live-conformance). 6 compiler crates promoted to validator runtime deps so the CLI can certify every target. Remaining: MCP parity, third-party adapter protocol, D1 normative spec, attestation/badge/dashboard, out-of-HTML-lens extractors |
...
| S60 (community launch / v1.1.0) | In progress — Slice 1 (README discoverability) + Slice 2 (publish prep: per-package metadata + LICENSE/README + workspaces + `publish-crates` & `publish-npm` jobs + Windows binary + `docs/launch/release-runbook.md`) + Slice 3 (repo polish) + Slice 5 (launch content drafts) shipped. **LICENSE compliance finding flagged** (workspace file is the Apache-2.0 header stub, not the full text). Remaining (all operator work, not in-repo): tag-push (Slice 6), GitHub Discussions + Discord setup (Slice 4), credential setup |
