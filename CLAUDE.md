...
| S79 (agent capability surface) | In progress — A1 (`voce skills`) + A3 (`voce graph`) shipped: reflected manifest + IR semantic graph (composition + 9 typed reference-edge kinds + state-machine reachability via BFS, contract-versioned). Remaining: A1 Slice 2 (MCP parity), A2 doctor, A4 JSON Schemas, A5 conformance runner, Part B |
